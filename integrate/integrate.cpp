...
double integrate(int n, double x0, double x1){
    ...
    for(int i = 1; i <= n ; ++i){
        sum1 = sum1 + f(x0 + step*i + step * 0.5);
        sum2 = sum2 + f(x0 + step*i);
    }
...
int main(){
    ...
    std::cout << "Integrate cos(x)*e^-x^2 from -1 to 3 (1000 steps): " << integrate(1000, -1, 3) << "\n";
    std::cout << "Difference from real value : " << realValue - integrate(1000, -1, 3) << "\n";
